...
-   **Sub-Hub Warehouse Roles and Dispatch Workflow**: Separates planning/approval from physical dispatch operations with strict scope-limited access controls. Logistics Officers and Managers plan and approve fulfilments with visibility across all hubs, while Warehouse Supervisors and Officers at Sub-Hubs have restricted access limited to their assigned location:
    -   **Access Restrictions**: Warehouse Supervisors/Officers are redirected to a dedicated warehouse dashboard upon login (no access to main ODPEM dashboard). Their inventory view shows ONLY items and stock levels from their assigned Sub-Hub with server-side validation preventing URL manipulation to view other hubs. Their needs list view displays ONLY needs lists (across all relevant statuses: Approved, Dispatched, Received, Completed) where their assigned Sub-Hub is designated as a fulfilment/dispatch hub. Needs lists from other Sub-Hubs, Main Hubs, or Agencies not assigned to their hub are hidden. Permission checks prevent direct URL access to unauthorized lists via the `can_view_needs_list` helper. All historical data (transaction history, stock reports) is strictly scoped to their assigned Sub-Hub with backend query-level filtering.
    -   **Navigation**: Warehouse roles see modified navigation menus showing Dashboard (links to warehouse dashboard), Inventory (scoped to their hub), and Needs Lists (scoped to their dispatch queue).
    -   **Workflow**: Warehouse users receive hub-scoped notifications when lists are approved for their hub, see ready-to-dispatch queues filtered by their assigned hub, and can only dispatch items from their assigned location. Logistics users can view approved lists but cannot dispatch - they see monitoring-only interfaces. The system tracks which warehouse user dispatched each order and validates hub assignments to prevent unauthorized dispatch operations.
    -   **History and Audit Scoping**: Transaction history (`/transactions`) and stock reports (`/reports/stock`) are filtered at the server/query level to show only data from the warehouse user's assigned Sub-Hub. Unassigned users or users assigned to non-SUB hubs receive error messages and are redirected. All filtering is enforced server-side to prevent direct URL or API access to other hubs' data.
